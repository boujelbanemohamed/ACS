...
const endpoints = {
  ...
  processing: {
    ...
    endpoints: [
      ...
      { method: 'POST', path: '/upload', description: 'Uploader un fichier CSV pour traitement. Multipart/form-data.', auth: 'jwt', roles: ['tous'], body: { file: 'fichier CSV (requis)', bankId: 'integer (requis pour super_admin)' } },
      { method: 'POST', path: '/process-url', description: 'Traiter un fichier CSV à partir d\'une URL distante.', auth: 'jwt', roles: ['tous'], body: { url: 'string (requis)', bankId: 'integer (requis pour super_admin)' } },
      ...
      { method: 'POST', path: '/process-manual', description: 'Enregistrer une entrée manuelle et générer le XML.', auth: 'jwt', roles: ['tous'], body: { pan: 'string', card_holder: 'string', phone: 'string', bankId: 'integer' } },
      { method: 'POST', path: '/call-api', description: 'Appeler une API externe pour récupérer des données (SSRF protégé).', auth: 'jwt', roles: ['tous'], body: { url: 'string (requis)', mapping: 'object (requis)', bankId: 'integer (requis pour super_admin)' } },
